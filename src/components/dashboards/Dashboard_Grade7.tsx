...
interface Dashboard_Grade7Props {
  user: UserProfile;
  language: Language;
  onLogout: () => void;
...
export function Dashboard_Grade7({ user, language, onLogout, onNavigateToLesson }: Dashboard_Grade7Props) {
  const [activeTab, setActiveTab] = useState('subjects');

  // Simplified welcome message without typing animation to improve performance
  const welcomeMessage = useMemo(() => {
    return language === 'en' ? `Grade 7 Dashboard - Welcome ${user.name}` : 
           language === 'hi' ? `कक्षा 7 डैशबोर्ड - स्वागत ${user.name}` : 
           `ଶ୍ରେଣୀ 7 ଡ୍ୟାସବୋର୍ଡ - ସ୍ୱାଗତ ${user.name}`;
  }, [language, user.name]);
            ...
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm text-gray-600 truncate">
                  {language === 'en' ? `Grade 7 • Level ${user.level}` : language === 'hi' ? `कक्षा 7 • स्तर ${user.level}` : `ଶ୍ରେଣୀ 7 • ସ୍ତର ${user.level}`}
                </p>
              </div>
            </div>
