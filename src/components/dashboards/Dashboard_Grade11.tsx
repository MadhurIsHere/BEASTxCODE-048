...
interface Dashboard_Grade11Props {
  user: UserProfile;
  language: Language;
  onLogout: () => void;
...
export function Dashboard_Grade11({ user, language, onLogout, onNavigateToLesson }: Dashboard_Grade11Props) {
  const [activeTab, setActiveTab] = useState('subjects');

  // Simplified welcome message without typing animation to improve performance
  const welcomeMessage = useMemo(() => {
    return language === 'en' ? `Grade 11 Dashboard - Welcome ${user.name}` : 
           language === 'hi' ? `कक्षा 11 डैशबोर्ड - स्वागत ${user.name}` : 
           `ଶ୍ରେଣୀ 11 ଡ୍ୟାସବୋର୍ଡ - ସ୍ୱାଗତ ${user.name}`;
  }, [language, user.name]);
  ...
  const subjects = useMemo(() => [
    {
      ...
      topics: ['Sets & Functions', 'Algebra', 'Calculus', 'Coordinate Geometry', 'Statistics'],
      progress: 85,
      level: 15,
      difficulty: 'Advanced',
      color: 'bg-blue-500'
            ...
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm text-gray-600 truncate">
                  {language === 'en' ? `Grade 11 • Level ${user.level}` : language === 'hi' ? `कक्षा 11 • स्तर ${user.level}` : `ଶ୍ରେଣୀ 11 • ସ୍ତର ${user.level}`}
                </p>
              </div>
            </div>
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-3">
                      ...
                      <Button 
                        ...
                        onClick={() => {
                          if (subject.id === 'advanced-mathematics') {
                            onNavigateToLesson?.('advanced-mathematics');
                          }
                        }}
                      >
